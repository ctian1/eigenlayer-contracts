...
contract DelegationManager is Initializable, OwnableUpgradeable, Pausable, DelegationManagerStorage, ReentrancyGuardUpgradeable {
    ...
    *******************************************************************************/
    ...
    /**
     * @dev Allows the staker, the staker's operator, or that operator's delegationApprover to undelegate
     *      a staker from their operator.
     *
     *      When called, it immediately removes all active shares and strategies associated with the staker 
     *      from both the staker and the operator. It then queues these shares and strategies for withdrawal,
     *      processing each strategy individually.
     *
     *      Note: This function can result in multiple withdrawal roots being generated, one for each strategy
     *      involved in the undelegation process. These roots are used to track and manage the queued withdrawals.
     *
     * @param staker The address of the staker to undelegate.
     * @return withdrawalRoots An array of bytes32 withdrawal roots, one for each strategy undelegated.
     *                         Each root is a unique identifier used to manage and track the withdrawal process
     *                         for the corresponding strategy's shares.
     */
